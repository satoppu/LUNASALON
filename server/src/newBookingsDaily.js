...
// revenue_confirmed_dateに絞ったSELECTだけを行うことで、テーブル全体の
// サイズに依存しない小さく一定のコストに抑えている(どちらも索引あり)。
 ...
 * 前」)。booking_date/revenue_confirmed_dateの範囲を絞ったSELECTのみなので、
 * offsetWindowsの大小やテーブル全体のサイズに関係なく一定のコストに収まる。
 *
 * 予約した日にその日の売上として計上し、後日キャンセルされた場合はキャンセル
 * が確定した日(revenue_confirmed_date、無ければbooking_date)の売上から
 * 差し引く — 実店舗の会計感覚に合わせた日ごとの分割計上(自社サイトの
 * booking_amountを保持する行のみ可能。他チャネルは本来の予約金額を保持して
 * いないため、従来通りbooking_date側に「取消」として計上する)。
 *
...
export function getNewBookingsDaily(offsetWindows = 0, store) {
  ...
  const rows = db
    .prepare(
      `SELECT booking_date AS date, revenue_confirmed_date, status, channel, revenue, booking_amount
       FROM transactions
       WHERE ((booking_date >= ? AND booking_date <= ?) OR (revenue_confirmed_date >= ? AND revenue_confirmed_date <= ?))
       ${storeClause}`
    )
    .all(...params);
  ...
  for (const r of rows) {
    if (r.status === SUBSCRIPTION_STATUS) {
      ...
      if (r.channel === OWN_SITE_CHANNEL && r.booking_amount != null) {
        ...
        const confirmDate = r.revenue_confirmed_date || r.date;
        if (inWindow(r.date)) {
 ...
 * 加えて、自社サイトのキャンセルでrevenue_confirmed_date=その日の行も含む
 * (キャンセル確定日側に「取消」として計上される行なので、明細にも出す)。
 */
  ...
  return db
    .prepare(
      `SELECT date, store, user_name, revenue, status, channel
       FROM transactions
       WHERE (booking_date = ? OR revenue_confirmed_date = ?) ${storeClause}
       ORDER BY user_name`
    )
    .all(...params);
}
