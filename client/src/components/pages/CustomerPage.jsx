...
export default function CustomerPage() {
  ...
  if (error) {
    return (
      <p className="text-sm" style={{ color: "#8C3B3B" }}>
        {error}
      </p>
    );
  }
  if (!state) {
    return <p style={{ color: "#8A857D" }}>読み込み中…</p>;
  }
    ...
    <>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={state.newCustomersByMonth} margin={{ bottom: 24 }}>
                ...
                <XAxis
                  ...
                  height={50}
                />
                <YAxis tick={{ fill: "#8A857D", fontSize: 12 }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip formatter={(v) => `${v}人`} />
                <Bar dataKey="count" fill="#345953" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div style={{ background: "#FFFFFF" }} className="p-4">
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={state.activeCustomersByMonth} margin={{ bottom: 24 }}>
                ...
                <YAxis tick={{ fill: "#8A857D", fontSize: 12 }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip formatter={(v) => `${v}人`} />
                <Bar dataKey="count" fill="#5E6FA0" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <p className="text-xs mt-2" style={{ color: "#8A857D" }}>
      ...
      <div>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-2" style={{ background: "#FFFFFF", border: "1px solid #E7E2DB" }}>
            <Search size={14} style={{ color: "#8A857D" }} />
            <input
              type="text"
              value={query}
            ...
            <tbody>
              {filtered.map((c) => (
                <tr
                  ...
                  style={{ borderBottom: "1px solid #F1EDE7" }}
                >
