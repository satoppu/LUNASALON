...
export default function CustomerDetailModal({ customer, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between px-6 py-5" style={{ borderBottom: "1px solid #EAE0E3" }}>
          <div>
            <h2 style={{ fontFamily: FONT_HEAD, color: "#262421" }} className="text-xl font-bold mb-1">
              {customer.user}
            </h2>
            <p style={{ color: "#8F7B82" }} className="text-sm">
              初回利用日: {customer.firstUseDate}
            </p>
          </div>
          <button onClick={onClose} style={{ color: "#8F7B82" }} aria-label="閉じる">
            <X size={20} />
          </button>
        </div>

        <div className="flex gap-8 px-6 py-4 flex-wrap" style={{ borderBottom: "1px solid #EAE0E3" }}>
          <div>
            <p style={{ fontFamily: FONT_HEAD, color: "#B5306A" }} className="text-xl font-bold">
              {customer.totalCount}
            </p>
          ...
          <div>
            <p style={{ fontFamily: FONT_HEAD, color: "#A83A56" }} className="text-xl font-bold">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid #EAE0E3" }}>
                <th className="text-left px-6 py-3 font-medium" style={{ color: "#8F7B82" }}>
                  年度
                </th>
                <th className="text-right px-6 py-3 font-medium" style={{ color: "#8F7B82" }}>
                  利用回数
                </th>
                <th className="text-right px-6 py-3 font-medium" style={{ color: "#8F7B82" }}>
                  キャンセル数
