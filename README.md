...
- 自社サイトの状態「未確定」(本日より先の日付で、まだ実施されていない予約。プラットフォーム側のUIでは「利用前」と表示される)は、ステータス「利用前」として売上に計上する(売上は「決済元金」+「割引金額」から算出。「利益」列はこの時点では空欄のため使用できない)。ただし稼働時間(`hours_used`)には含めない — まだ実際に利用されていないため。日付が過ぎて次回以降のエクスポートで「利用済み」に変われば、同じ決済IDの行が自動的に更新され(下記の通り)、稼働時間も計上されるようになる。Instabaseの場合は利用開始日時が本日より後の行を引き続き取り込まない(現状はステータスラベルが自社サイトほど明確でないため)。「利用済み」→利用済み、「キャンセル(顧客)」相当は利益(またはInstabaseの予約金額)>0なら「キャンセル(返金あり)」・0なら「キャンセル(顧客)」、「キャンセル(オーナー)」はそのまま新しいステータスとして保存(売上・稼働時間ともに計上対象外)。
